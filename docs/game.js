...
document.addEventListener("DOMContentLoaded", () => {
    ...
    try {
      if (isMobile) {
        ...
        try {
          ...
          alert("Failed to switch to Pharos Testnet. Please check your wallet settings.");
          return false;
        }
  ...
  function moveLoop() {
    if (gameActive) {
      ...
      newBottom = Math.min(window.innerHeight - cr.height, newBottom);
      character.style.bottom = newBottom + "px";
      const lhRect = r(lighthouse);
      const paddingLeft = lhRect.width * 0.50;
  ...
  async function showPaymentModal(callback) {
    try {
      ...
      const m = modal(`<div style="text-align:center">
        ...
        ${btn("Pay & Start", "go", isMobile ? "width:100%" : "")}
      </div>`);
  ...
  async function finishLevel(reached) {
    ...
    if (reached) {
      html += `<div style="margin-top:10px">${btn("Claim Reward", "btnClaim", isMobile ? "width:100%" : "")}</div>`;
    }
    if (reached && gotAll && currentLevel < 3) {
      html += `<div style="margin-top:6px">${btn("Next Level", "btnNext", isMobile ? "width:100%" : "")}</div>`;
    } else if (reached && gotAll && currentLevel === 3) {
      html += `<div style="margin-top:6px;font-size:16px"> You completed all levels!</div>
              <div style="margin-top:6px">${btn("Play Again (Level 1)", "btnAgain", isMobile ? "width:100%" : "")}</div>`;
    } else {
      html += `<div style="margin-top:6px">${btn("Restart Level", "btnRestart", isMobile ? "width:100%" : "")}</div>`;
    }
    html += `<div style="margin-top:6px">${btn("Leaderboard", "btnLeader", isMobile ? "width:100%" : "")}</div>`;
    html += `<div style="margin-top:6px">${btn("Main Menu", "btnMenu", isMobile ? "width:100%" : "")}</div>`;
    html += `</div>`;
    const m = modal(html);
    if (reached) {
    ...
    if (toRestart) toRestart.onclick = () => { m.close(); showPaymentModal(() => { startLevel(); }); };
  ...
  function gameOver(byWave = true) {
    ...
    const m = modal(`<div style="text-align:center">
      ...
      ${btn("Restart from Level 1", "btnR", isMobile ? "width:100%" : "")}
      ${btn("Leaderboard", "btnLeader", isMobile ? "width:100%" : "")}
      ${btn("Main Menu", "btnMenu", isMobile ? "width:100%" : "")}
      </div>`);
